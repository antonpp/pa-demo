...
import { getSystemInstructions } from './prompts/system_instructions.js';
import 'dotenv/config'; // This will load .env if it exists, but won't fail if it doesn't
...
async function main() {
  ...
  await io.on('connection', async function (socket: Socket) {
    ...
    socket.on('contentUpdateText', function (text: string) {
      session.sendClientContent({ turns: `<instructions>${text}</instructions>`, turnComplete: true });
    });
